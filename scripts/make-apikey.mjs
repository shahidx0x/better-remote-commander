...
const raw = (await r.text()).match(/value="(sesrdp_ak_[^"]+)"/)?.[1];
...
console.log('POST /mcp initialize ->', /ses-rdp/.test(mcp) ? 'ok' : mcp.slice(0, 80));
const bad = await fetch(`${base}/api/devices`, { headers: { authorization: 'Bearer sesrdp_ak_wrong' } });
