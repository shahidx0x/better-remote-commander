...
if (!cfg.adminPassword) { console.error('SES_RDP_ADMIN_PASSWORD is required'); process.exit(2); }
if (!cfg.sessionSecret) {
  cfg.sessionSecret = randomBytes(32).toString('hex');
  console.warn('SES_RDP_SESSION_SECRET not set: using a random secret (browser sessions reset on restart)');
}
...
relay.http.listen(cfg.port, cfg.host, () => {
  log('info', `ses-rdp-relay ${cfg.relayVersion} listening on ${cfg.host}:${cfg.port}`);
